...
const svgMap = {
  // TrailTribe Icons
  '🥾': '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>', // Changed to a trail path or map pin. Wait, let's use map-pin.
...
// Also fix index.html
const indexMap = {
  '👋': '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14.5 10c0-3.5-3-5-5-5s-5 1.5-5 5v5h10v-5z"></path><path d="M14.5 15h4.5a2 2 0 0 0 2-2v-3a2 2 0 0 0-2-2h-2"></path><path d="M4.5 15H2v3a2 2 0 0 0 2 2h2v-5z"></path></svg>'
};

function replaceEmojis(filePath, mappings) {
  if (!fs.existsSync(filePath)) return;
  let content = fs.readFileSync(filePath, 'utf8');
  let changed = false;

  for (const [emoji, svg] of Object.entries(mappings)) {
    // Specifically target emojis wrapped in classes if possible to prevent accidental replaces, 
    // but the emojis in these files are all acting as icons anyway.
    // Using a regex to replace the emoji if it is inside a tag or standalone
...
replaceEmojis('trailtribe-case-study.html', svgMap);
replaceEmojis('index.html', indexMap);
